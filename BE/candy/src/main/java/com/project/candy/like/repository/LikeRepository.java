...
//  Optional<Like> readLikeSelectedByUser(Long beerId, long userId);
  ...
  @Query(nativeQuery = true, value = "select * from `like` where (`like`.user_id = :user_id) and (`like`.beer_id = :beer_id) and (`like`.is_delete = false)")
  Like readByUserAndBeerAndIsDeleteFalse(@Param(value = "user_id") long userId, @Param(value = "beer_id") long beerId);
}
